...
var clean = function (path, done) {
    ...
    return del(path, options, done).then(function (paths) { return console.log(paths); });
};
var replaceText = function (path) {
    if (!path) {
        console.log('No path');
    }
    var match = /@\[app([A-Z]\w+)\](\S+\.html)/g.exec(path);
...
gulp.task('buildClean', function (done) {
    ...
    var source = keys.map(function (key) { return (pubPaths[key] + "/**"); });
...
gulp.task('buildScripts', function () {
    ...
    var orginalSourcePaths = function () {
        ...
        var keys = Object.keys(sysJsCfg.map);
        //console.log(keys);
...
gulp.task('getTypings', function () {
    var check = function (file) { return file.path.indexOf('.d.ts') > -1; };
...
gulp.task('rebuildApp', gulp.series('buildClean', 'buildScripts', 'buildStyles', 'getIndexHtml', 'getTypings', 'buildFonts', 'buildImages', 'buildTemplates', 'buildApp'));
gulp.task('buildAppWatch', gulp.series('rebuildApp', function () { return gulp.watch(['{app,content,scripts,views}/*.{js,html,less}', '!scripts/vendor/bower/**'], gulp.series('rebuildApp')); }));
