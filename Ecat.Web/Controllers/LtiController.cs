                            ...
                            foreach (var member in groupMembers)
                            {
                                ...
                                foreach (var peer in peers)
                                {
                                    ...
                                    if (wrkgrp.MpCategory == MpGroupCategory.Wg1 && member.StudentId != peer.StudentId)
                                    {
                                        ...
                                        writer.WriteLine(
                                            $"INSERT INTO StudSpComment(CourseId,WorkGroupId,AuthorPersonId,RecipientPersonId,RequestAnonymity,CreatedDate,ModifiedDate,CommentText)Values({comment.CourseId},{comment.WorkGroupId},{comment.AuthorPersonId},{comment.RecipientPersonId},'{comment.RequestAnonymity}','{comment.CreatedDate.ToUniversalTime()}','{comment.ModifiedDate?.ToUniversalTime()}','{comment.CommentText}');");
